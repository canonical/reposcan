...
see [run a scan](run-a-scan.md#record-a-scan). `-n/--limit` and `--wrap` tune
...
- `--allow-code-execution`: let dependency resolution build source packages when
  ...
  and without it a legacy Poetry project is not resolved at all.
