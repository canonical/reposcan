...
def test_native_binary_tarball_extracts_finds_and_symlinks() -> None:
    lines = "\n".join(_TRUFFLEHOG.script_install(_LINUX, _ROOT))
...
def test_multi_file_download_is_kept_whole_and_symlinked() -> None:
    ...
    # from its siblings).
    lines = "\n".join(_GO_SDK.script_install(_LINUX, _ROOT))
...
def test_pypi_writes_the_hash_lock_inline_then_installs_from_it() -> None:
    lines = "\n".join(_SEMGREP.script_install(_LINUX, _ROOT))
...
def test_go_tool_pins_the_module_and_builds_with_the_sdk_go() -> None:
    lines = "\n".join(_GOVULNCHECK.script_install(_LINUX, _ROOT))
...
def test_locate_executable_names_the_executable_the_symlink_points_to() -> None:
    # bin/<name> for binaries and Go tools; bin/<entrypoint> for a PyPI tool.
    assert _GO_SDK.locate_executable(_ROOT) == "/opt/tools/bin/go"
    assert _GOVULNCHECK.locate_executable(_ROOT) == "/opt/tools/bin/govulncheck"
    assert _SEMGREP.locate_executable(_ROOT) == "/opt/tools/bin/semgrep"  # entrypoint


def test_plan_installs_pulls_in_prerequisites_ordered_and_deduped() -> None:
    # Request the dependents plus one explicit prerequisite; plan_installs pulls
    # in the missing prerequisites, de-dupes, and orders each tool after what it needs.
    plan = plan_installs([_SEMGREP, _GOVULNCHECK, _UV], _LINUX, _ROOT)
