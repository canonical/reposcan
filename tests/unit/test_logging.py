...
def _capture_emitted(*records: tuple[str, bool], tty: bool) -> str:
...
def test_a_transient_record_is_redrawn_in_place_then_erased_by_the_next_line() -> None:
    written = _capture_emitted(("one", True), ("two", True), ("done", False), tty=True)
...
def test_a_transient_record_is_dropped_when_stderr_is_not_a_terminal() -> None:
    ...
    written = _capture_emitted(("one", True), ("done", False), tty=False)
