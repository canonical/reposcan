...
def _build_tree(*commands: type) -> type[Group]:
    attrs = {"name": "app", "help": "root", "subcommands": commands}
    return type("Root", (Group,), attrs)


def _build_cli(*commands: type) -> Cli:
    return Cli("app", root=_build_tree(*commands), base=_Base)  # no resolver


def test_run_dispatches_to_a_command_and_returns_its_exit_code() -> None:
    assert _build_cli(_Echo).run(["echo", "--code", "5"]) == 5
    assert _build_cli(_Echo).run(["echo"]) == 0  # code takes its default
...
def test_a_group_without_a_chosen_subcommand_is_a_usage_error() -> None:
    ...
    cli = Cli("app", root=_build_tree(inner), base=_Base)
    assert cli.run(["grp"]) == 2  # a subcommand is required


def test_a_help_request_prints_and_returns_0() -> None:
    assert _build_cli(_Echo).run(["--help"]) == 0
    assert _build_cli(_Echo).run(["echo", "--help"]) == 0
...
def test_an_unmet_requirement_is_rejected_before_the_command_runs() -> None:
    ...
    assert _build_cli(_Need).run(["need", "--depth", "3"]) == 2
...
def test_with_no_resolver_values_come_from_the_command_line_and_defaults() -> None:
    ...
    _build_cli(_Cmd).run(["cmd", "--mode", "rich"])
...
def test_a_repeatable_option_collects_every_occurrence() -> None:
    ...
    assert _build_cli(_Tag).run(["tag", "--tags", "a", "--tags=b"]) == 0
    assert seen == [["a", "b"]]
...
def test_a_required_option_left_out_is_a_usage_error() -> None:
    ...
    assert _build_cli(_Needs).run(["needs"]) == 2
    assert _build_cli(_Needs).run(["needs", "--target", "somewhere"]) == 0
