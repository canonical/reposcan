...
from tests.unit.actions.helpers import patch_scan, sbom_artifact
...
def test_sbom_exits_zero_and_prints_a_component_table() -> None:
    ...
    with tempfile.TemporaryDirectory() as repo:
        ...
        with patch_scan(sbom_cmd, sbom_artifact(3)), redirect_stdout(out):
            code = action.run()
...
def test_sbom_forwards_dependency_options_to_the_scan() -> None:
    ...
    with tempfile.TemporaryDirectory() as repo:
        ...
        with (
            patch_scan(sbom_cmd, sbom_artifact(0), captured=captured),
...
def test_sbom_reports_a_scan_failure_as_one() -> None:
    with tempfile.TemporaryDirectory() as repo:
        action = sbom_cmd.SbomCommand(path=repo)
        with patch_scan(sbom_cmd, Failure(reason="cdxgen crashed")):
            code = action.run()
    assert code == 1
