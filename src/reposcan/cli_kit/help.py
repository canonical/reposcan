...
def render(node: type[Action | Group], scope: list[Param], prog: str) -> str:
    """Render help for `node`: usage, description, options, and subcommands."""
    ...
    for param in positionals:
        parts.append(_format_usage_token(param))
    ...
    if options:
        lines += ["", "options:"]
        rendered = [(_format_option_flags(p), p) for p in options]
...
def _format_usage_token(param: Param) -> str:
    if param.remainder:
        return "[-- args ...]"
    if param.many:
        return f"[{param.name} ...]"
...
def _format_option_flags(param: Param) -> str:
