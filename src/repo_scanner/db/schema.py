...
ANALYSIS = TableSchema(
    ...
    insert=(
        "INSERT INTO analysis (uuid, produced_by, project_id, started_at, "
        "finished_at, reposcan_version, target_name, branch, commit_sha, dirty, "
        "shallow, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ),
    select=(
        "SELECT analysis_id, uuid, produced_by, project_id, started_at, finished_at, "
        "target_name, branch, commit_sha, dirty, shallow, status "
        "FROM analysis ORDER BY analysis_id"
    ),
)
...
SCAN = TableSchema(
    ...
    insert=(
        "INSERT INTO scan (analysis_id, category, artifact_kind, started_at, "
        "finished_at, status, artifact_shell) VALUES (?, ?, ?, ?, ?, ?, ?)"
    ),
    select=(
        "SELECT scan_id, analysis_id, category, artifact_kind, started_at, "
        "finished_at, status, artifact_shell FROM scan ORDER BY scan_id"
    ),
)
...
INVOCATION = TableSchema(
    ...
    select=(
        ...
        "exit_code, successful FROM invocation WHERE scan_id = ? "
...
ISSUE_REPORT = TableSchema(
    ...
    insert="INSERT INTO issue_report VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    select=(
        "SELECT result_json FROM issue_report WHERE scan_id = ? ORDER BY result_index"
    ),
)
...
COMPONENT_REPORT = TableSchema(
    ...
    insert="INSERT INTO component_report VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    select=(
        "SELECT component_json FROM component_report "
        "WHERE scan_id = ? ORDER BY component_index"
    ),
)
...
ISSUE_FINGERPRINT = TableSchema(
    ...
    create=(
        ...
        "name TEXT NOT NULL, "
        "value TEXT NOT NULL, "
        "PRIMARY KEY (issue_id, complete, name)) WITHOUT ROWID"
    ),
    insert="INSERT INTO issue_fingerprint VALUES (?, ?, ?, ?)",
    select=(
        "SELECT complete, name, value FROM issue_fingerprint WHERE issue_id = ? "
...
SELECT_ANALYSIS_BY_UUID = "SELECT analysis_id FROM analysis WHERE uuid = ?"
SELECT_LATEST_ANALYSIS = (
    "SELECT analysis_id FROM analysis ORDER BY analysis_id DESC LIMIT 1"
)
SELECT_LATEST_ANALYSIS_FOR_PROJECT = (
    "SELECT analysis_id FROM analysis WHERE project_id = ? "
    "ORDER BY analysis_id DESC LIMIT 1"
)
SELECT_ANALYSES = (
    "SELECT analysis_id, project_id, uuid, started_at, status, produced_by, "
    "commit_sha, "
    "branch, dirty, shallow FROM analysis ORDER BY analysis_id"
)
SELECT_CATEGORIES_FOR_ANALYSIS = (
    "SELECT category FROM scan WHERE analysis_id = ? ORDER BY scan_id"
)
SELECT_SCANS_FOR_ANALYSIS = (
    "SELECT scan_id, category, artifact_kind, artifact_shell FROM scan "
    "WHERE analysis_id = ? ORDER BY scan_id"
)
...
SELECT_CANDIDATES = (
    ...
    "FROM issue i "
    "JOIN issue_report r ON r.issue_id = i.issue_id "
    "AND r.scan_id = (SELECT MAX(scan_id) FROM issue_report WHERE issue_id = "
    "i.issue_id) "
    "LEFT JOIN issue_fingerprint p ON p.issue_id = i.issue_id "
    "WHERE i.project_id = ? AND i.category = ? AND i.rule = ? "
    "ORDER BY i.issue_id"
)
INDEXES = (
    # Candidate lookup filters on all three, and rule is the selective one.
    "CREATE INDEX IF NOT EXISTS issue_scope ON issue(project_id, category, rule)",
    "CREATE INDEX IF NOT EXISTS issue_report_issue ON issue_report(issue_id, scan_id)",
...
UPSERT_ISSUE_FINGERPRINT = (
    ...
    "VALUES (?, ?, ?, ?) "
    "ON CONFLICT(issue_id, complete, name) DO UPDATE SET value = excluded.value"
)
...
SELECT_ISSUE_FINGERPRINTS = (
    ...
    "FROM issue_fingerprint f JOIN issue i ON i.issue_id = f.issue_id "
